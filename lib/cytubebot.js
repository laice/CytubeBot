...
const discordConfig = require('./dconfig.js');
...
discordClient.on('ready', () => {
	...
	for(let keys in discordClient) {
		console.log(keys);
	}
	console.log(('presences'), discordClient.presences);
    console.log(('users'), discordClient.users);
    console.log(('guilds'), discordClient.guilds);
    console.log(('shard'), discordClient.shard);
    console.log(('channels'), discordClient.channels);
    console.log(('options'), discordClient.options);
	gpChannel = discordClient.channels.get("name", "general");
	console.log(gpChannel);
...
CytubeBot.prototype.handleChangeMedia = function(data) {
    ...
    if(discordConnected && gpChannel) {

        discordClient.sendMessage(gpChannel, `Now playing ${this.currentMedia["title"]} in https://goon.pub`);
    }
};
