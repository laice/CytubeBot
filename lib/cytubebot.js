...
function pubgStats(key, username, callback) {
	const request = require('request');
	console.log(key, username);
	const opt = {
		url: `https://api.pubg.com/shards/pc-na/players?filter[playerNames]=${username}`,
    headers: {
      "Authorization": key,
...
function matchStats(key, match, player, callback) {
	const request = require('request');
	console.log('looking up match ', match);
	console.log('player' , player);
  const opt = {
    url: `https://api.pubg.com/shards/pc-na/matches/${match}`,
    headers: {
      "Authorization": key,
      "Accept": "application/vnd.api+json"
    }
  ...
  function cb(e, res, body) {
  	if(e) {
		...
		if(!e) {
			...
			let parts = match.included;
			//console.log(util.inspect(match.data, false, null));
			//console.log(util.inspect(parts, false, null));
			parts.forEach((part, i, arr) => {
				//console.log(JSON.stringify(part.attributes.stats));
				if(part.attributes.stats) {
					if(part.attributes.stats.playerId) {
						let id =  part.attributes.stats.playerId;
						//console.log('got player id ', id);
            if(id == player) {
              callback(JSON.stringify(part))
            }
					}
				}
...
function CytubeBot(config) {
	...
	// Discord
  this.discordConfig = require('./dconfig.js');

	this.discordHook = new Webhook(this.discordConfig.hook);
	this.discordClient = new Discord.Client();


	this.discordConnected = false;
	this.gpChannel = null;
	let discordClient = this.discordClient;
    discordClient.login(this.discordConfig.token);
    discordClient.on('ready', () => {
        console.log('discord connected');
        //console.log('client', JSON.stringify(discordClient.guilds));
        this.discordConnected = true;
        //gpChannel = discordClient.servers.get("name", "goon.pub").defaultChannel;
        //console.log(`channel`, gpChannel);


        //console.log(('channels'), this.discordClient.channels);

        this.gpChannel = this.discordClient.channels.get('name', 'general');
        console.log(this.gpChannel);
    ...
    discordClient.on('message', (msg) => {
		...
		// activate this to censor discord links
        // if(msg.content.search("https://discord.gg") != -1) {
        //     msg.delete()
        //         .then(console.log('deleted external discord link'))
        //         .catch(console.error);
        // }

		//console.log(cJSON.stringify(msg.author));


        // if(msg.author.member) {
		// 	console.log(msg.author.member);
		// }

      if(msg.author.username === "jynn") {
        if(msg.author.discriminator === '5618') {
          if(msg.content === '!updates') {
            console.log('saw updates', msg.channel);
            this.gpChannel = msg.channel;
            return;
          }
          if(msg.content === '!test') {
            msg.delete()
			...
			let prefix = this.discordConfig.prefix;

			let pre = msg.content.slice(0,1);
			if(pre === prefix) {
				let ctnt = msg.content.split(" ");
				//console.log(ctnt);
				ctnt[0] = ctnt[0].substr(1);

				let cmd = ctnt.shift();
				switch(cmd) {
					case 'nowplaying':
					case 'np':
            msg.reply(`Now playing: ${this.currentMedia["title"]} - https://goon.pub`);
            return;
					case 'pubg':
						pubgStats(this.discordConfig.pubgKey, ctnt.join(","), (stats) => {
							let id = stats.data[0].id;
							let matches = stats.data[0].relationships.matches.data;

							msg.reply(`ID: ${id}, Matches Played: ${matches.length}`);
							let lastMatch = matches[matches.length-1];
							console.log(lastMatch);
							matchStats(this.discordConfig.pubgKey, lastMatch.id, id, (match) => {
								console.log(util.inspect(match, false, null));

								msg.reply(`Last match stats for ${match.name}
														Kill Place: ${match.killPlace}
														Win Place: ${match.winPlace}
														Kills: ${match.kills}
														Assists: ${match.assists}
														Headshots: ${match.headshotKills}
														Revives: ${match.revives}
														Road Kills: ${match.roadKills}
														Team Kills: ${match.teamKills}
														Vehicles Destroyed: ${match.vehicleDestroys}																											
														Kill Streaks: ${match.killStreaks}
														Last Kill Points: ${$match.lastKillPoints}
														Kill Points: ${match.killPoints}
														Last Win Points: ${match.lastWinPoints}
														Win Points: ${match.winPoints}
														Longest Kill: ${match.longestKill}
														Most Damage: ${match.mostDamage}
														Death Type: ${match.deathType}
														Time Survived: ${match.timeSurvived}
														Walk Distance: ${match.walkDistance}
														Swim Distance: ${match.swimDistance}
														Ride Distance: ${match.rideDistance}
														Weapons Aquired: ${match.weaponsAcquired}											
        ...
        split.forEach((item, i, arr) => {
          //console.log(item);
          let res = item.search('https://youtu.be');

          let long;
          if (res < 0) {
            res = item.search('https://www.youtube.com');
            long = true;
          }


          if (res > -1) {
            //console.log('found link: ', item);
            // let id
            // if(long) {
            //     id = item.split("v=")[1];
            //     let ampPos = id.indexOf('&');
            // if(ampPos != -1) {
            ...
            if (id[2] && id[2].length == 11) {
              console.log(id);
              this.addVideo("yt", id[2], null, true, "end");
              msg.reply('Added to the https://goon.pub queue! Come watch!');
              return;
            }
    ...
    discordClient.on('guildMemberAdd', (member) => {
        member.guild.defaultChannel.send(`Welcome, ${member}! Join us @ https://goon.pub !nowplaying: ${this.currentMedia["title"]}`);
    });
...
CytubeBot.prototype.cleanDatabaseVideos = function() {
	var self = this
		...
		self.db.getVideos(num, videos => {
			...
			videos.forEach(video => {
				...
				self.videoLookupLimiter.removeTokens(1, () => {
					count++
					validator.validate(self, video.id, video.type, video.title, postValidate)
				})
			})
		})
	})
...
CytubeBot.prototype.handleChangeMedia = function(data) {
	...
	// publish to discord
	this.discordHook.info("goon.pub", `Now playing ${this.currentMedia['title']} on https://goon.pub/`);
...
CytubeBot.prototype.talk = function(message, callback) {
	...
	this.talkLimiter.removeTokens(1, function() {
		Cleverbot.prepare(function() {
			self.talkBot.write(message, function(response) {
